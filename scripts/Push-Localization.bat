...
if not exist "%GIT_EXE%" (
    ...
    echo   %ESC%[33m       Установите Git через меню Установка [1] → [2]%ESC%[0m
    pause
    popd
    exit /b 1
)
...
if not exist "%REPO_DIR%\.git" (
    ...
    echo   %ESC%[33m       Клонируйте репозиторий через меню Установка [1] → [3]%ESC%[0m
    pause
    popd
    exit /b 1
)
...
if not "!CURRENT_BRANCH!"=="ru-localization" (
    echo   %ESC%[1;33m→%ESC%[0m %ESC%[1mПереключение на ветку ru-localization...%ESC%[0m
