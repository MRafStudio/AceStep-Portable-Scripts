...
REM ============================================================================
REM   Авто-создание / обновление Config.ini (СОХРАНЕНИЕ СТАРЫХ НАСТРОЕК!)
REM ============================================================================
...
if not exist "%CONFIG_FILE%" (
    ...
    REM Проверяем, есть ли CUDA_VERSION (добавлено в новых версиях)
    findstr /B /C:"CUDA_VERSION=" "%CONFIG_FILE%" >nul 2>nul
    if !errorlevel! neq 0 set "CONFIG_NEED_CREATE=1"
)
...
if "!CONFIG_NEED_CREATE!"=="1" (
    if exist "%CONFIG_FILE%" (
        ...
        REM Сохраняем старые значения
        set "OLD_LANGUAGE="
        set "OLD_CURRENT_MODEL="
        set "OLD_LM_MODEL="
        set "OLD_AUTO_OPEN_BROWSER="
        set "OLD_LAUNCH_METHOD="
        set "OLD_CUDA_VERSION="
        
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"LANGUAGE=" "%CONFIG_FILE%"') do set "OLD_LANGUAGE=%%b"
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"CURRENT_MODEL=" "%CONFIG_FILE%"') do set "OLD_CURRENT_MODEL=%%b"
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"LM_MODEL=" "%CONFIG_FILE%"') do set "OLD_LM_MODEL=%%b"
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"AUTO_OPEN_BROWSER=" "%CONFIG_FILE%"') do set "OLD_AUTO_OPEN_BROWSER=%%b"
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"LAUNCH_METHOD=" "%CONFIG_FILE%"') do set "OLD_LAUNCH_METHOD=%%b"
        for /f "tokens=1,2 delims==" %%a in ('findstr /B /C:"CUDA_VERSION=" "%CONFIG_FILE%"') do set "OLD_CUDA_VERSION=%%b"
        
        REM Чистим пробелы
        set "OLD_LANGUAGE=%OLD_LANGUAGE: =%"
        set "OLD_CURRENT_MODEL=%OLD_CURRENT_MODEL: =%"
        set "OLD_LM_MODEL=%OLD_LM_MODEL: =%"
        set "OLD_AUTO_OPEN_BROWSER=%OLD_AUTO_OPEN_BROWSER: =%"
        set "OLD_LAUNCH_METHOD=%OLD_LAUNCH_METHOD: =%"
        set "OLD_CUDA_VERSION=%OLD_CUDA_VERSION: =%"
        
        REM Дефолты если пусто
        if "!OLD_LANGUAGE!"=="" set "OLD_LANGUAGE=ru"
        if "!OLD_CURRENT_MODEL!"=="" set "OLD_CURRENT_MODEL=turbo"
        if "!OLD_LM_MODEL!"=="" set "OLD_LM_MODEL=acestep-5Hz-lm-1.7B"
        if "!OLD_AUTO_OPEN_BROWSER!"=="" set "OLD_AUTO_OPEN_BROWSER=1"
        if "!OLD_LAUNCH_METHOD!"=="" set "OLD_LAUNCH_METHOD=gradio"
        if "!OLD_CUDA_VERSION!"=="" set "OLD_CUDA_VERSION=12.8"
        
        REM Валидация модели
        call "%SCRIPTS_DIR%\MapModel.bat" "!OLD_CURRENT_MODEL!"
        if "!REAL_MODEL!"=="acestep-v15-turbo" if not "!OLD_CURRENT_MODEL!"=="turbo" (
            REM MapModel вернул дефолт — значит старая модель невалидна
            set "OLD_CURRENT_MODEL=turbo"
        )
        
        call "%SCRIPTS_DIR%\CreateConfig.bat" "!OLD_LANGUAGE!" "!OLD_CURRENT_MODEL!" "!OLD_LM_MODEL!" "!OLD_AUTO_OPEN_BROWSER!" "!OLD_LAUNCH_METHOD!" "!OLD_CUDA_VERSION!"
...
REM ============================================================================
REM   Валидация и авто-фикс модели (через MapModel)
REM ============================================================================
call "%SCRIPTS_DIR%\MapModel.bat" "%CURRENT_MODEL%"

REM Если MapModel вернул дефолт (turbo), но CURRENT_MODEL был другим — значит невалидно
...
REM Проверка: если REAL_MODEL всё ещё turbo, но CURRENT_MODEL не turbo — невалидная модель
...
REM Повторная валидация через MapModel
call "%SCRIPTS_DIR%\MapModel.bat" "%CURRENT_MODEL%"
if "%REAL_MODEL%"=="acestep-v15-turbo" if not "%CURRENT_MODEL%"=="turbo" (
    set "CURRENT_MODEL=turbo"
    call "%SCRIPTS_DIR%\MapModel.bat" "turbo"
...
echo     Модель DiT: %ESC%[1;33m%CURRENT_MODEL%%ESC%[0m %ESC%[2m^(%REAL_MODEL%, %MODEL_SIZE%, %MODEL_VRAM%, %MODEL_STEPS% шагов^)%ESC%[0m
...
if "!INSTALLED_COUNT!"=="3" (
    echo   %ESC%[1;37m[*]%ESC%[0m %ESC%[1mЗапуск AceStep-1.5%ESC%[0m %ESC%[2m^(http://127.0.0.1:7860^)%ESC%[0m
) else (
